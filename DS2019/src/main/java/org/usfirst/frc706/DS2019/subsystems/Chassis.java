...
public class Chassis extends Subsystem {
   ...
   // private final static TalonSRX leftDriveThree = RobotMap.chassisleftDriveThree;
   ...
   // private final static TalonSRX rightDriveThree = RobotMap.chassisrightDriveThree;
    ...
    //		rightDriveThree.set(ControlMode.PercentOutput, right*Constants.Chassis.MULTIPLIER);
    ...
    //		leftDriveThree.set(ControlMode.PercentOutput, -1*left*Constants.Chassis.MULTIPLIER);
    	}
	...
	public static void mecanumMove(double x, double y, double rotation) {
		...
		/*if (x==0){
			...
			if (x>=0)
				...
				theta = Math.PI; //don't reinvent the wheel
		}else */
...
/* Aidaan
		double radius = Math.hypot(xSpeed, ySpeed);//drive Speed accounting for x and y

		if(radius > 1) radius = 1;
		double driveAngle = -1*Math.atan(ySpeed / xSpeed);//drive angle in radians
			
		if (xSpeed <= 0) driveAngle += Math.PI;
		if (driveAngle <= 0)driveAngle += 2*Math.PI;
		...
		double rbd = 0;
			...
			if(Math.abs(rotation) < Constants.OI.uncivilizedXBoxController){
				rotation = 0;                 //Fix XBox Problems
			}

			if(radius > Math.hypot(Constants.OI.uncivilizedXBoxController, Constants.OI.uncivilizedXBoxController)){
				lfd = radius * Math.sin(driveAngle + (Math.PI/4)) + rotation;
				rfd = radius * -Math.cos(driveAngle  + (Math.PI/4)) - rotation;
				lbd = radius * -Math.cos(driveAngle + (Math.PI/4)) + rotation;
				rbd = radius * Math.sin(driveAngle + (Math.PI/4)) - rotation;
			
			}else if(rotation > Math.abs(Constants.OI.uncivilizedXBoxController)){
				lfd = rotation;
				rfd = -rotation;
				lbd = rotation;
				rbd = -rotation;
		...
		Robot.dashboard.putNumber("lfd", lfd);
		Robot.dashboard.putNumber("rfd", rfd);
		Robot.dashboard.putNumber("lbd", lbd);
		Robot.dashboard.putNumber("rbd", rbd);
		Robot.dashboard.putNumber("rotation", rotation);
		
    	rightDriveOne.set(ControlMode.PercentOutput, rfd*Constants.Chassis.MULTIPLIER);
		rightDriveTwo.set(ControlMode.PercentOutput, rbd*Constants.Chassis.MULTIPLIER);
		leftDriveOne.set(ControlMode.PercentOutput, -lfd*Constants.Chassis.MULTIPLIER);
		leftDriveTwo.set(ControlMode.PercentOutput, -lbd*Constants.Chassis.MULTIPLIER);
		*/
    }
