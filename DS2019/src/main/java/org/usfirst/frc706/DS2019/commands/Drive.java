...
public class Drive extends Command {
    ...
    //	if (!Robot.auto) {
    		//if (Robot.oi.sonicOutput.getAverageValue() < 320) {DriverStation.reportError("DISTANCE IS REALLY CLOSE",false);}
    		//else if (Robot.oi.sonicOutput.getAverageValue() < 380) {DriverStation.reportError("DISTANCE IS CLOSE",false);}
    		//else {DriverStation.reportError("DISTANCE IS FAR",false);}
    		//if (Robot.oi.sonicOutput.getValue() > 900 || (Robot.oi.rightJoy.getY()>0&&Robot.oi.leftJoy.getY()>0)) {
				...
				if(Constants.Chassis.useXbox){//pull values from XBox remote
					...
					//if (Math.abs(0-YPower)<=Constants.OI.uncivilizedXBoxController) YPower = 0; //Fixes xbox controller values sticking
				...
				Chassis.mecanumMove(Math.abs(XPower) > 0.2 ? XPower : 0, Math.abs(YPower) > 0.2 ? YPower : 0, rotation);
				Elevator.percentPower(.5); //set test motor to 50%
    		/*}
    		else {	
    			if (Robot.oi.sonicOutput.getValue() > 600) {
    			Chassis.move(-Robot.oi.rightJoy.getY(),-Robot.oi.leftJoy.getY());
    			}
    			else {
    				Chassis.move(0, 0)
    			}
    		}*/
    		//DriverStation.reportError("SUPERSONIC VOLTAGE: " + Robot.oi.sonicOutput.getAverageValue(), false);
    //	}
    }
